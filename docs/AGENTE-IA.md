...
`wsl/profiles/agent.psd1` usa a política `latest`. `INSTALAR.cmd` e `ATUALIZAR.cmd` resolvem a release estável atual pela API oficial do GitHub e exigem o digest SHA-256 publicado para o asset. O manifesto local registra a versão resolvida, o hash do arquivo e o hash do binário instalado. Se a API, o digest ou a verificação falhar, o binário não é substituído.
...
- chaves SSH;
- arquivos `.env` reais;
- discos de VM e dados gerados pelo agente.
